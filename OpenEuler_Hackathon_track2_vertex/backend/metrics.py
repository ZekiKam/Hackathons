...
from ws_app import globalMetrics, globalMissRates
...
def read_results_json():
    algorithm_mapping = {
        "DQN": {"id": "dqn", "title": "Deep Q Network"},
        "Random": {"id": "random", "title": "Random"},
        "LRU": {"id": "lru", "title": "Least Recently Used"}, 
        "LFU": {"id": "lfu", "title": "Least Frequently Used"},
        "MRU": {"id": "mru", "title": "Most Recently Used"}
    }
    
    try:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        results_path = os.path.join(script_dir, "../ai/results.json")

        with open(results_path, "r") as f:
            raw_data = json.load(f)  # This is now an array
            
        # Transform the array data to match frontend format
        formatted_data = []
...
def main():
    ...
    last_results_read = time.time()

    while True:
        t0 = time.time()
        ts = datetime.datetime.now(datetime.UTC).isoformat()
        
        if time.time() - last_results_read >= 1.0:
            results_data = read_results_json()
            
            if results_data:
                global globalMissRates
                globalMissRates.clear()
                globalMissRates.append(results_data)
