...
globalMissRates = deque(maxlen=1)
...
async def collect_stats():
    return {
        ...
        "cores": getCoresData(),
        "info": [getInfo()],
        "miss_rates": list(globalMissRates)[-1] if globalMissRates else [] 
    }

def getData(key):
    return [d[key] for d in globalMetrics if key in d]

def getMissRate(algorithm_id):
    if not globalMissRates:
        return -1
    
    # Get the latest miss rates data
    latest_data = globalMissRates[-1]  # Get the most recent entry
