...
function Dashboard() {
    ...
    const [missRates, setMissRates] = useState([
        { id: "dqn", title: "Deep Q Network", data: -1 },
        { id: "random", title: "Random", data: -1 },
        { id: "lru", title: "Least Recently Used", data: -1 },
        { id: "lfu", title: "Least Frequently Used", data: -1 },
        { id: "mru", title: "Most Recently Used", data: -1 },
    ]);
    ...
    return (
        <div className="p-4 grid grid-cols-3 auto-rows-fr gap-4 w-full h-full">
            ...
            {missRates.length > 0 && missRates[0].data != -1 && (
                <BarChartBox 
                    id="missrates" 
                    title={`Predicted Cache Miss Rates | Best (${missRates.reduce((best, current) => 
                        current.data < best.data ? current : best
                    ).title})`}
                    percentage={Math.min(...missRates.map(m => m.data))} 
                    data={missRates.map(rate => ({ 
                        id: rate.id, 
                        value: rate.data,
                        title: rate.title  // Add this line
                    }))} 
                />
            )}
